...
class SeleniumBrowser {
    ...
    {
        return $this->driver->findElement(WebDriverBy::tagName($string));
    }


    public function findByCss(string $string)
    {
        return $this->driver->findElement(WebDriverBy::cssSelector($string));
    }

    public function listByCssInElement(string $string, WebDriverElement $element){
        return $element->findElements(WebDriverBy::cssSelector($string));
    }

    public function findByXpath(string $string)
    {
        return $this->driver->findElement(WebDriverBy::xpath($string));
    }

    public function hasByCss(string $string)
    {
        if (count($this->driver->findElements(WebDriverBy::cssSelector($string))) > 0) {
            return true;
        } else {
            return false;
        }
    ...
    {
        if (count($this->driver->findElements(WebDriverBy::xpath($string))) > 0) {
            return true;
        } else {
            return false;
        }
